...
!---------------------------------------------
  ...
  subroutine solver(nsources, sglob, samp, nreceivers, rglob, ramp, &
    ...
    allocate(Stiff(3*NGLOB,3*NGLOB))
    ...
    !Init stiff matrix
    Stiff(:,:)=0.0
    print *, "Stiff,rank:",rank
    call MPI_Barrier(comm,ierr)
    call init_stiff_matrix_PSV(Stiff)
    print *,"Stiff_PSV:",rank
    ...
    !
    do itime = 1, NSTEP
      if(rank.eq.0.and.(mod(itime,1).eq.0.or.itime.eq.1))  print *, "itime=", itime
      ...
      do i = 1, 3
        ...
        accel(i,:) = 0.
      enddo
!      print '(a,3g12.5,i)',"max veloc pre", maxval(displ),maxval(veloc),maxval(accel),rank
...
!      print *, "max residual pre:",maxval(rhs),rank
      ...
      !print *,'begin, absorb:', rank
      !print *, "simul_type:", simul_type, rank
      !print *, "absorb flag:", absorb_flag, rank
      call MPI_Barrier(comm,ierr)
      if(ABSORB_FLAG) then
!        print *,"add absorb boundary condition:", rank
      ...
      call MPI_Barrier(comm,ierr)
!      print *, "max residual absorb:",maxval(rhs),rank
...
!        print *,"add source:", rank, sglob(1)
      ...
      call MPI_Barrier(comm,ierr)
!      print *, "max residual source:",maxval(rhs),rank
      ...
      !              ibool_interfaces, nibool_interfaces, &
      !              my_neighbours, rank, comm)
      !print *, "assemble mass:",rank
      call MPI_Barrier(comm,ierr)

      call assemble_MPI_vector(rhs,3*nglob,ID, &
                      ninterface, max_ibool_interfaces_size,&
                      ibool_interfaces, nibool_interfaces, &
!                      !buffer_send_faces_vector,&
!                      !buffer_recv_faces_vector,&
                      my_neighbours, rank, comm)
      !print *, "assemble vector:",rank
      call MPI_Barrier(comm,ierr)
      ...
      enddo
!      print *,"max veloc post", maxval(displ), maxval(veloc),maxval(accel),rank
